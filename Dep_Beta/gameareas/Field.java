...
public class Field extends Area {
	...
	public Field() {
		super("Field");
		areatype = AT_FIELD;
		Encounters.put(new Monster(), new Double(0.6));  //for now...
	}
	
	public Field(String name_arg) {
		super(name_arg);
		areatype = AT_FIELD;
		Encounters.put(new Monster(), new Double(0.6));  //for now...s
	}
	...
	*/
	...
	public void event(gChar gch) {
		...
		for(i=0; i<3; i++) { //3 possibilities for encounters
			...
			for( Monster mon : Encounters.keySet() ) {
				if( r < (Encounters.get(mon)).doubleValue() ) {
					Battler.battle(gch, mon);
					break;
				}
			}
